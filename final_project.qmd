...
dfm_matrix_train <- as.matrix(dfm_df_train)
dfm_label_built <- as.numeric(docvars(cities_dfm_train, "sys_built_b"))-1
dfm_label_nature <- as.numeric(docvars(cities_dfm_train, "sys_nature_b"))-1
dfm_label_social <- as.numeric(docvars(cities_dfm_train, "sys_social_b"))-1
dfm_label_gov <- as.numeric(docvars(cities_dfm_train, "sys_gov_b"))-1

built_xgb <- xgboost(
  data = dfm_matrix_train,
  label = dfm_label_built,
...
built_pred_xgb <- round(predict(built_xgb, newdata = as.matrix(dfm_df_test)))
built_score_xgb <- mean(built_pred_xgb == as.numeric(docvars(cities_dfm_test, "sys_built_b"))-1); 1-built_score_xgb

nature_pred <- round(predict(nature_xgb, newdata = as.matrix(dfm_df_test)))
nature_score <- mean(nature_pred == docvars(cities_dfm_test, "sys_nature_b")); 1-nature_score

social_pred <- predict(social_xgb, newdata = dfm_df_test)
social_score <- mean(social_pred == docvars(cities_dfm_test, "sys_social_b")); 1-social_score
...
Here, I move on to predicting for the test set. One other point worth noting is that LDA technically assigned each document a probability of belonging to each topic, but for comparability with RF (which in this case is predicting a single class for each document) I assign each document to the topic with the highest probability.
...
So these error rates are pretty poor. Two of them, built infrastrucure and governance, are as bad (well, a tiny bit worse) than random, and the other two, nature and social equity, and about double the error rate for random forests. On one hand, performance worse than random is not great, but 25-35% error rates, given that this is completely unsupervised, is arguably not that bad. I think what it indicates more than anything else is that nature and social equity terms (at least defined as the topics they are) are more distinctive and easier to pick out with LDA. 
...
Now with the seeded model, we see that performance improves across the board by a few percentage points. Notably, built infrastructure and governance are both performing better better than random, but not by much. Performance for nature and social also improves, for social equity in particular. However the error rates for these are still categories are still roughly double the error rates for random forests. 
